...
function PPF_CLIENT_MARK_OFFLINE_AND_LOGOUT() {
  try {
    const current = JSON.parse(localStorage.getItem("currentUser") || "null");
    if (current) {
      const stats = JSON.parse(localStorage.getItem("userStats") || "{}");
      const key = current.nickname || current.username;
      if (key && stats[key]) {
        stats[key].online = false;
        stats[key].lastLogout = new Date().toISOString();
...
window.PPF_LOGOUT_AND_SYNC = PPF_CLIENT_MARK_OFFLINE_AND_LOGOUT;

if (!currentPatient) {
  alert("No se han encontrado tus datos de cliente.");
  PPF_CLIENT_MARK_OFFLINE_AND_LOGOUT();
}
